...
import TourCard, { TourItem } from "@/components/section/tours/TourCard";
...
interface ToursExploreLayoutProps {
  ...
  currentItems: TourItem[];
  totalPages: number;
  startIndex: number;
  endIndex: number;
}

export default function ToursExploreLayout({
  activeCategory,
  onCategoryChange,
  currentPage,
  onPageChange,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="w-full px-4 md:px-8 lg:px-0 max-w-6xl mx-auto py-8">
        ...
        <ExplorePageHeader
          title="Explore Tours"
          description="Discover amazing tours across Sri Lanka"
        />
        ...
        {currentItems.length === 0 && (
          <NoResults message="No tours found for this category." />
        )}
      </div>
    </div>
  );
